...
const actions = {
  ...
  setUpUserAuth({ commit }) {
    return new Promise((resolve, reject) => {
      let unsubAuthListenerFun = onAuthStateChanged(
        auth,
        (user) => {
...
const getters = {
  ...
  isAuthenticated(_state, getters) {
    return getters.isGeoAuthenticated || getters.isUserAuthenticated;
  },
  isGeoAuthenticated(state) {
    return state.authinit && !!state.geoauth;
  },
  isUserAuthenticated(state) {
    return state.authinit && !!state.user;
  },
  isInitialized(state) {
